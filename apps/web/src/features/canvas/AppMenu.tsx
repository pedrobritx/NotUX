...
function PopoverMenu({ children, onEnter, onLeave }: { children: ReactNode; onEnter(): void; onLeave(): void; }) {
  return (
    <div className="menu" onMouseEnter={onEnter} onMouseLeave={onLeave}>
      {children}
    </div>
  );
}
...
export function AppMenu({ boardId, client, owned }: AppMenuProps) {
  ...
  useShapeStore((s) => s.revision);

  type MenuType = "file" | "edit" | "view" | "arrange" | null;
  ...
  const activeTitle = pages.find((p) => p.id === activePageId)?.title ?? "Untitled";
  const hasSelection = selection.size > 0;

  function openMenu(m: MenuType) {
  ...
  function selectAll() {
    const ids = useShapeStore.getState().listShapes(activePageId).map((s) => s.id);
  ...
  async function runExport(scope: "current" | "all") {
    ...
    const pgs = scope === "current" ? all.filter((p) => p.id === active) : all;
    setExporting(true);
    try {
      await exportBoardToPdf({ pages: pgs, filename: "board.pdf" });
    } catch (e) {
    ...
    <>
      <GlassPanel className="app-menu" aria-label="Menu">
        <button
          ...
          onMouseEnter={() => activeMenu && openMenu("file")}
          onClick={() => activeMenu === "file" ? setActiveMenu(null) : openMenu("file")}
        >
          File
        </button>
        <button
          ref={editBtnRef}
          className={`app-menu__btn app-menu__text-btn ${activeMenu === "edit" ? "app-menu__text-btn--active" : ""}`}
          onMouseEnter={() => activeMenu && openMenu("edit")}
          onClick={() => activeMenu === "edit" ? setActiveMenu(null) : openMenu("edit")}
        >
          Edit
        </button>
        <button
          ref={viewBtnRef}
          className={`app-menu__btn app-menu__text-btn ${activeMenu === "view" ? "app-menu__text-btn--active" : ""}`}
          onMouseEnter={() => activeMenu && openMenu("view")}
          onClick={() => activeMenu === "view" ? setActiveMenu(null) : openMenu("view")}
        >
          View
        </button>
        <button
          ref={arrangeBtnRef}
          className={`app-menu__btn app-menu__text-btn ${activeMenu === "arrange" ? "app-menu__text-btn--active" : ""}`}
          onMouseEnter={() => activeMenu && openMenu("arrange")}
          onClick={() => activeMenu === "arrange" ? setActiveMenu(null) : openMenu("arrange")}
        >
          Arrange
        </button>
